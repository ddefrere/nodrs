PRO merging_window_v5, cx, cy, cxref, cyref, dim, resize_only=resize_only
...
obj_ttmp=readfits(datadir+'img_'+tg_name_bas+'_dc.fits',header)
	...
	writefits, datadir+'img_'+tg_name_dyn+'_dc.fits', obj_ttmp, header
	endif else begin
	tg_name_dyn=tg_name_dyn+'_rsz'
	writefits, datadir+'img_'+tg_name_dyn+'_dc.fits', obj_ttmp, header
	endelse
	...
	if file_test(datadir+'img_'+tg_name_dyn+'_dc.fits') eq 0 then begin
	obj_ttmp=readfits(datadir+'tg_'+tg_name_ori+'_obj'+'_dc.fits',header)
	nobj=(size(obj_ttmp))[3]
	obj_ttmp=obj_ttmp[cx-dim/2:cx+dim/2-1,cy-dim/2:cy+dim/2-1,0:nobj-1]
	ref_ttmp=readfits(datadir+'tg_'+tg_name_ori+'_ref'+'_dc.fits',header2)
	...
	writefits, datadir+'img_'+tg_name_dyn+'_dc.fits', obj_tmp, header
	endif else begin
	obj_ttmp=readfits(datadir+'tg_'+tg_name_dyn+'_dc.fits',header)
