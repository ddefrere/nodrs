...
;	Version 1.1b, 14-SEP-2013, DM: Keck version
;-

PRO CALL_PCA_KECK, cx, cy, camera_filter, fwhm, plsc, dim, sigma, klip, ZONE=zone, DELTA=delta, SMART=smart, FILTER=filter, PHOTOMETRY=photometry, DISPLAY=display, NOCURVE=nocurve
...
normal=median(normal_v)
ENDIF ELSE BEGIN
normal=1.0;modify to any required value
ENDELSE
...
rin_init = 5 ; inner radius where PCA starts, in resolution elements (fwhm) 
...
MERGING_WINDOW_V5B, cx, cy, cx, cy, dim, /resize_only
...
CONTRAST_CURVE_V71, pca_img_median, dim/2, dim/2, fwhm, plsc, dim, sigma, /GAUSSFILT, BOX=box, DISPLAY=display
...
;Injection of fake companions
FCP_V84, fwhm, pseudo_sigma_fcp, rad_c, n_br, plsc, dim, camera_filter, right_handed, $ 
         NORMAL=normal, /GAUSSFILT, /KECK, /ADI, BOX=box, DISPLAY=display
...
PREP_PS & DEVICE, FILENAME=procdir+'contrast_map.eps', /ENCAPS, /COLOR, BITS_PER_PIXEL=8, /PORTRAIT
LOADCT, 2, /SILENT 
...
PREP_PS & DEVICE, FILENAME=procdir+'PCA_SNR_map.eps', /ENCAPS, /COLOR, BITS_PER_PIXEL=8
LOADCT, 2, /SILENT 
...
OPLOT, [-dim/2,dim/2]*plsc, [0,0]*plsc, LINESTYLE=1 & OPLOT, [0,0]*plsc, [-dim/2,dim/2]*plsc, LINESTYLE=1
DEVICE, /CLOSE
DEVICE, FILENAME=procdir+'PCA_SNR_fcp_map.eps', /ENCAPS, /COLOR, BITS_PER_PIXEL=8
