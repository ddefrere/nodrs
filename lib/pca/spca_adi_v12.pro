...
;   
; CALLING SEQUENCE:
;   Result = SPCA_ADI_V11(cx_init, cy_init, dim, dim_init, fwhm, plsc, truncate_pca, rin_init, step_init, delta, right_handed, $
...
;     - for each annulus, each frame is considered separately to build the PSF library
...
FUNCTION SPCA_ADI_V12, cx_init, cy_init, dim, dim_init, fwhm, plsc, truncate_pca, rin_init, step_init, delta, right_handed, $
                       onezone=onezone, normal=normal, display=display, verbose=verbose, wcs=wcs
...
for iann = 0,n_annuli-1 do begin
	;Define annulus #i
	in = rin + floor((iann*step_init*fwhm_rounded));-dr/2+1 ; inside limit of the zone s in pixel
	IF KEYWORD_SET(onezone) THEN BEGIN
	  out = rout
	  step = out - in
	ENDIF ELSE BEGIN
  	step = (rin + floor(((iann+1)*step_init*fwhm_rounded))) - in ; radial thickness of the zone s in pixel (can be varying)
  	out = in + step
  ENDELSE
	print, in, out, step
  index_annulus=where( (rho lt out) AND (rho ge in) )
  npix_annulus_i = n_elements(index_annulus)
 	obj_annulus_i= fltarr(npix_annulus_i,nobj) ; create a 2D matrix that will contain the info of the considered annulus for each frame
	...
	ELSE delta_deg = delta / (in/fwhm_rounded) / !dpi*180 ; computed at the inner edge of annulus for safety
	
	if keyword_set(verbose) then print, 'Cutting and saving annulus of '+strcompress(step)+'pix ('+strcompress(step/fwhm_rounded)+$
	...
	for fr=0, nobj-1 do obj_annulus_i[*,fr] = (obj[*,*,fr])[index_annulus]
  ...
  data=0 & covMatrix=0 & eigenval=0 & eigenvect=0 
	...
	for fr=0, nobj-1 do begin
	  obj_annulus_i_mean[fr]=mean(obj_annulus_i[*,fr])
		obj_annulus_i[*,fr]=obj_annulus_i[*,fr]-obj_annulus_i_mean[fr]
	endfor
	
	; Doing the sPCA analysis frame by frame
 	for fr=0,nobj-1 do begin 
  	index_ref = where((paral lt paral[fr]-delta_deg) or (paral gt paral[fr]+delta_deg))  ; Dimitri: why did you use 'ABS' here???
  	nref = n_elements(index_ref)
  	print, strcompress(iann+1)+'/'+strcompress(n_annuli), fr, nref
  	data = obj_annulus_i[*,index_ref]
    ...
    ;Principal components 
   	...
   	for k=0,truncate_pca-1 do begin
  		...
  		coeff=total(obj_annulus_i[*,fr]*reform(pc[k,*]))
  		;print, coeff
  		supref=supref+coeff*reform(pc[k,*])
  	endfor
  	
   	objtmp[index_annulus,fr]=obj_annulus_i[*,fr]-supref;+obj_annulus_i_mean[fr]
	endfor
