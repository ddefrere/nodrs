...
;   Version 3.9, 11-Dec-2013, DD: improved computation speed of derotation
;-


FUNCTION PCA_ADI_V38, cx_init, cy_init, dim, dim_init, fwhm, truncate_pca, Na, rin_init, right_handed, $ 
                      NORMAL=normal, DISPLAY=display, VERBOSE=verbose, WCS=wcs
...
if keyword_set(verbose) then print, 'Derotate by: ', paral[i]
	if right_handed eq 1 then paral[i]=-paral[i]
		objt[0,0,i]=rot(objt[*,*,i],-paral[i],1.0,dim/2,dim/2,cubic=-0.5,/pivot)
endfor
