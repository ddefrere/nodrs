...
;   
; CALLING SEQUENCE:
;   FCP_V83, fwhm, sigma, rad_c, n_br, plsc, dim, camera_filter, right_handed, ADI=adi, KECK=keck, NACO=naco, NICI=nici, $
...
;	Version 8.4, 14-SEP-2013, DM: add Keck filters
;-


PRO FCP_V84, fwhm, sigma, rad_c, n_br, plsc, dim, camera_filter, right_handed, ADI=adi, KECK=keck, NACO=naco, NICI=nici, $
             EVAL_NOISE=eval_noise, NORMAL=normal, GAUSSFILT=gaussfilt, BOX=box, AGPM=agpm, STUDENT=student, DISPLAY=display
...
;------------------------------
r = ROUTINE_INFO('fcp_v84', /SOURCE)
path = STRMID(r.path,0,STRLEN(r.path)-STRLEN('fcp_v84.pro'))
...
IF KEYWORD_SET(keck) THEN BEGIN
  ...
  if camera_filter eq 'Kp' then fcp=readfits(path+'fake_companion_keck_K.fits')
  if camera_filter eq 'Ks' then fcp=readfits(path+'fake_companion_keck_K.fits')
  if camera_filter eq 'K' then fcp=readfits(path+'fake_companion_keck_K.fits')
...
IF KEYWORD_SET(adi) then begin ; derotate the companion position in case of an ADI sequence
  paral = readfits(procdir+'vec_'+tg_name_bas+'_paral.fits')
