...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
function pca_rdi_v3, cx, cy, cxref, cyref, dim, fwhm, plsc, truncate_pca_low, truncate_pca_high, Na, rin_init, fake_companion, filter, display=display, recenter=recenter, normal=normal, remove_mean=remove_mean, verbose=verbose
...
obj[*,*,i]=obj[*,*,i]-median(obj[*,*,i],filter*fwhm)
endfor
ref=obj_tmp[*,*,nobj:nref+nobj-1]
for i=0, nref-1 do begin
ref[*,*,i]=ref[*,*,i]-median(ref[*,*,i],filter*fwhm)
endfor
endif else begin
obj=obj_tmp[*,*,0:nobj-1]
ref=obj_tmp[*,*,nobj:nref+nobj-1]
...
if total(fake_companion) ne 0 then begin
	...
	print, '**********************************************************************************'
	fcp_t=readfits('fake_companion_nici_H_s18.fits')
	sz=(size(fcp_t))[2]
	fcp=fltarr(dim,dim)
...
proj_obj_dc=fltarr(dim,dim,truncate_pca)
objt=fltarr(dim,dim,nobj)
test_th=fltarr(dim,dim,nobj)

supreft=fltarr(dim,dim,nobj)
...
for x=0,floor(dim/Na)-1 do begin
	...
	t1=systime(1)
	covMatrix = transpose(data) ## data
	print, systime(1)-t1
		t1=systime(1)
	covMatrixb = matrix_multiply(data, data, /ATRANSPOSE)
		print, systime(1)-t1
	stop
	eigenval = EIGENQL(covMatrix, EIGENVECTORS=eigenvect, EigenIndex=index, /DOUBLE)
	if keyword_set(verbose) then print, 'PCA Eigenvalues: ', eigenval
	
 	res=fltarr(bs*bs,nref)
	for k=0,nref-1 do begin
	res[*,k]=data ## eigenvect[*,k] / sqrt(eigenval[k])
	endfor
   	res=Reform((res), bs, bs, nref)
   	res=res(*,*,truncate_pca_low:truncate_pca_high)	  
	proj_obj_dc[x*bs:x*bs+bs-1,y*bs:y*bs+bs-1,*]=res[*,*,*]
	...
	supref_test=fltarr(bs,bs)
		...
		coeff=total(reform(obj[x*bs:x*bs+bs-1,y*bs:y*bs+bs-1,i])*reform(proj_obj_dc[x*bs:x*bs+bs-1,y*bs:y*bs+bs-1,k]))
		supref=supref+coeff*reform(proj_obj_dc[x*bs:x*bs+bs-1,y*bs:y*bs+bs-1,k])
		if total(fake_companion) ne 0 then begin
		coeff_test=total(reform(test[x*bs:x*bs+bs-1,y*bs:y*bs+bs-1,i])*reform(proj_obj_dc[x*bs:x*bs+bs-1,y*bs:y*bs+bs-1,k]))
		supref_test=supref_test+coeff_test*reform(proj_obj_dc[x*bs:x*bs+bs-1,y*bs:y*bs+bs-1,k])
		endif
		endfor
	supreft[x*bs:x*bs+bs-1,y*bs:y*bs+bs-1,i]=supref
