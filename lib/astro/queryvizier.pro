...
;         Now handle case where column names appear three times (e.g., Gezari 1999) D.D. April 2017
;-
 ...
 case strmid(fmt[i],0,1) of 
  ...
  endcase
   ...
   if i EQ 0 then   info = create_struct(colname[0], cval) else begin
	   ...
	   if where(tag_names(info) eq strupcase(colname[i]) + '_2') ge 0 then $
	     colname[i] = colname[i] + '_3'
