...
PRO REDUCE_HOSTS5, date, BAD_IDX=bad_idx, BCKG_IDX=bckg_idx,  DARK_IDX=dark_idx, DATA_IDX=data_idx, FLAT_IDX=flat_idx, NOD_IDX=nod_idx, OB_IDX=ob_idx, $          ; Date to reduce and file index 
...
; Config files
IF NOT KEYWORD_SET(QUICK) THEN cfg_file = 'hosts5.cfg' ELSE cfg_file = 'quicknull.cfg'
...
FOR i=0, N_ELEMENTS(cfg_file)-1 DO BEGIN
  ...
  IF KEYWORD_SET(calib_null) THEN BEGIN
    ...
    NULL_CALIB, date, cfg_file[i], REMOVE_OB=remove_ob, PLOT=plot, VERSION=l1version, /NO_INSET, /CALPOB              ; L2 fits will be per OB
