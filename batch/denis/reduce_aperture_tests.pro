...
PRO REDUCE_APERTURE_TESTS, date, BAD_IDX=bad_idx, BCKG_IDX=bckg_idx,  DARK_IDX=dark_idx, DATA_IDX=data_idx, FLAT_IDX=flat_idx, NOD_IDX=nod_idx, OB_IDX=ob_idx, $          ; Date to reduce and file index 
...
plot     = 0         ; Plot data on screen if larger than 1
verbose  = 1
...
; Config files
;IF NOT KEYWORD_SET(QUICK) THEN cfg_file = 'tests_aperture_rad.cfg' ELSE cfg_file = 'quicknull.cfg'
IF NOT KEYWORD_SET(QUICK) THEN cfg_file = 'test_one_aperture.cfg' ELSE cfg_file = 'quicknull.cfg'
;cfg_file = ['tests_aperture_rad_6.cfg', 'tests_aperture_rad_8.cfg', 'tests_aperture_rad_11.cfg', 'tests_aperture_rad_15.cfg', 'tests_aperture_rad_20.cfg', 'tests_aperture_rad_26.cfg', 'tests_aperture_rad_33.cfg'];,'hosts_a36.cfg']
...
FOR i=0, N_ELEMENTS(cfg_file)-1 DO BEGIN
  ...
  IF KEYWORD_SET(fit) THEN BEGIN
    NULL_FIT, date, cfg_file[i], PLOT=plot, /PS, L1TYPE='PT-BIAS'
