...
;
; PURPOSE
;   Main batch routine to reduce LBTI Fizeau data (see LBTI_DRS.pro for more information).
;   Modifiy "fizeau.cfg" under "nodrs/cfg/" in order to specify the instrument (LMIRCam or NOMIC).
;
; MANDATORY INPUT
...
PRO REDUCE_FIZEAU, date, BAD_IDX=bad_idx, BCKG_IDX=bckg_idx,  DARK_IDX=dark_idx, DATA_IDX=data_idx, FLAT_IDX=flat_idx, NOD_IDX=nod_idx, $     ; Date to reduce and file index 
                  MASTERLOG=masterlog, RENEW=renew, SKIP_ADI=skip_adi, SKIP_FLX=skip_flx, SKIP_RED=skip_red                                   ; Control what to do
