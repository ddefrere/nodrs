...
PRO REDUCE_TAUCET, date, BAD_IDX=bad_idx, BCKG_IDX=bckg_idx,  DARK_IDX=dark_idx, DATA_IDX=data_idx, FLAT_IDX=flat_idx, NOD_IDX=nod_idx, OB_IDX=ob_idx, $          ; Date to reduce and file index 
...
; Config files
IF NOT KEYWORD_SET(QUICK) THEN cfg_file = 'taucet.cfg' ELSE cfg_file = 'quicknull.cfg'
