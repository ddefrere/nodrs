...
drs = CREATE_STRUCT(drs, 'VERSION', 9.2, 'DATE', '28-JUL-2018', 'DATE_OBS', date_obs)
...
FOR i = 0, n_pt-1 DO BEGIN
  ...
  max_apr[i] = MIN(xcen_sx[idx] MOD cnf.X_CHAN) < MIN(ycen_sx[idx] MOD cnf.Y_CHAN) < MIN(cnf.X_CHAN - (xcen_sx[idx] MOD cnf.X_CHAN)) < MIN(cnf.Y_CHAN - (ycen_sx[idx] MOD cnf.Y_CHAN))
...
FOR i_d = 0, n_data-1 DO BEGIN
  ...
  idx_pho = WHERE(pt_id EQ pt_id[i_d] AND cfg_id EQ cfg_id[i_d] AND obstype EQ 0, n_pho)
  IF n_pho LE 0 THEN BEGIN
    ...
    idx_pho = WHERE(objname EQ objname[i_d] AND cfg_id EQ cfg_id[i_d] AND obstype EQ 0, n_pho)
  ENDIF 
        ...
        CASE hdr_data.header.obstype OF 
          ...
          2: BEGIN ; Nulling frames 
               ...
               IF n_bck GE 1 THEN BEGIN
                 file_bck = file_id[idx_bck]                                                     
                 idx_bck  = idx_bck[WHERE(ABS(fid_cur-file_bck) EQ MIN(ABS(fid_cur-file_bck)), n_ok)]  ; keep only the closest one(s)
...
FOR i_o = 0, n_obj-1 DO BEGIN
  ...
  FOR i_lam = 0, n_lam-1 DO BEGIN  
    ...
    file = FILE_SEARCH(pth.l1fits_path + date_obs + drs.dir_label + pth.sep, search_string + '*NULL*_IMG.fits', COUNT=n0)
